...
export const WorkBleed: React.FC<{
  ...
  return (
    <div
      className="h-screen absolute flex flex-col items-center justify-center text-3xl lg:text-3xl md:w-screen-1/2"
      style={{
...
export const WorkBleedR: React.FC<{
  progress: number;
  children: React.ReactNode;
}> = ({ children, progress }) => {
  let translateY = Math.max(0, 50 - progress * 3 * 50);
  ...
  return (
    <div
      className="h-screen absolute flex flex-col items-center justify-center text-3xl lg:text-3xl bleed-r-container md:w-screen-1/2 md:right-0"
      style={{
        transform: `translateY(${translateY}px)`,
      }}
    >
...
export const WorkBleedProject: React.FC<{
  progress: number;
  children: React.ReactNode;
}> = ({ children, progress }) => {
  let translateY = Math.max(0, 50 - progress * 3 * 50);
